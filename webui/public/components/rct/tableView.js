...
import mcRow from './table/mc/mc-row.js';
import mcHeader from './table/mc/mc-header.js';
...
function mc(model) {
    return h('div',
        mcHeader(() => model.changeRecordsVisibility()),
        model.RCTdataFetched ? model.RCTCurentContent.map(item => mcRow(model, item)) : 'loading data'
    );
}
